...
function App() {
  ...
  const [pressedbtn, setPressedbtn] = React.useState("Drums"); 
    return (
        <div id="drum-machine" className="bg-info min-vh-100 text-white">
            <div className="text-center">
                <h2>Drum Machine</h2>
                {audioClips.map((clip) => (
                    <Pad key={clip.id} clip={clip} volume={volume} button={setPressedbtn}/>
                ))}
                <br/>
                <div id="display" >{pressedbtn}</div>
                <h4>Volume</h4>
...
function Pad({clip, volume, button}) {
